...
for fold in range(1):
    ...
    for config in configs:
        ...
        test = pd.read_csv(f'{WORKING_DIR}/ckpt/{config}/train_fold{fold+1}.csv')
    ...
    df = spinal.merge(nfn[['path', 'fold']+pred_cols], on=['path', 'fold'])  # 我自己手動將 fold_x fold_y 刪掉 -> 留下一個 fold(用 fold_x 填值)
    ...
    df.merge(fold_df, on='study_id').to_csv(f'train_for_sagittal_level_cl_v1_for_train_spinal_nfn_fold1.csv', index=False)
...
# 5. 輸出 CSV
df.to_csv(f'train_for_sagittal_level_cl_v1_for_train_spinal_nfn_fold1.csv', index=False)'''
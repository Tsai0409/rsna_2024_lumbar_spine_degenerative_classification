...
class RSNA2024ModelV1(nn.Module):
    def __init__(self, base_model, n_instance=5, num_classes=75, in_features=1024,
        drop_rate=0, drop_rate_last=0.3):
        super(RSNA2024ModelV1, self).__init__()
        self.encoder = base_model
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.lstm = nn.LSTM(in_features, in_features//2, num_layers=1, dropout=drop_rate, bidirectional=True, batch_first=True)
        self.head = nn.Sequential(
            #nn.Linear(in_features, in_features//2),
            #nn.BatchNorm1d(in_features//2),
            nn.Dropout(drop_rate_last),
            #nn.LeakyReLU(0.1),
            nn.Linear(in_features, num_classes//n_instance),
        )

    def forward(self, x):  # (bs, nslice, ch, sz, sz)
        bs, n, ch, w, h = x.shape # 8, 4, 3, 384, 384
        x = x.view(bs * n, ch, w, h) # 32, 3, 384, 384
        x = self.encoder(x) # 32, 2048, 20, 20
        x = self.avgpool(x)

        x = x.view(bs, n, -1)
        x, _ = self.lstm(x)
        
        x = x.contiguous().view(bs * n, -1)
        
        x = self.head(x)
        
        x = x.view(bs, -1).contiguous()
        
        x = torch.nan_to_num(x, 0, 0, 0)
        return x
class RSNA2024ModelV3(nn.Module):
    def __init__(self, base_model, n_instance=5, num_classes=75, in_features=1024,
        drop_rate=0, drop_rate_last=0.3):
        super(RSNA2024ModelV3, self).__init__()
        self.encoder = base_model
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.lstm = nn.LSTM(in_features, 256, num_layers=2, dropout=drop_rate, bidirectional=True, batch_first=True)
        self.head = nn.Sequential(
            nn.Linear(512, 256),
            nn.BatchNorm1d(256),
            nn.Dropout(0.3),
...
class RSNA2024ModelV4(nn.Module):
    def __init__(self, base_model, n_instance=5, num_classes=75, in_features=1024,
        drop_rate=0, drop_rate_last=0.3):
        super(RSNA2024ModelV4, self).__init__()
        self.encoder = base_model
        self.avgpool = nn.AdaptiveAvgPool2d(1)
        self.gru = nn.GRU(in_features, 256, num_layers=1, dropout=drop_rate, bidirectional=True, batch_first=True)
...
class RSNA2024ModelV2(nn.Module):
    def __init__(self, base_model, n_instance=5, num_classes=75, in_features=1024,
        drop_rate=0, drop_rate_last=0.3):
        super(RSNA2024ModelV1, self).__init__()
        self.encoder = base_model
...
class RSNA2ndModel(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.encoder(x) # 32, 2048, 20, 20
        if self.swin:
            bs2, w2, h2, ch2 = x.shape # for swin transformer
        else:
...
class RsnaGru(nn.Module):
    def __init__(
        ...
        hidden_dim = 256
        self.gru = nn.GRU(nc*2, hidden_dim, 2, batch_first=True, bidirectional=True)
        self.fc = nn.Linear(hidden_dim*2*n_instance, num_classes)

    def forward(self, x):
        bs, n, ch, w, h = x.shape # 2, 5, 3, 384, 384
        x = x.view(bs * n, ch, w, h) # -> 10, 3, 384, 384
        x = self.encoder(x) # -> 10, 768, 12, 12
        x = self.pool(x) # -> 10, 1536, 1, 1
        x = x.view(bs, n, -1) # -> 2, 5, 1536
        x, _ = self.gru(x) # -> 2, 5, 512
...
class RsnaLstm(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.pool(x) # -> 10, 1536, 1, 1
        x = x.view(bs, n, -1) # -> 2, 5, 1536
        x, _ = self.lstm(x) # -> 2, 5, 512
        x = x.reshape(bs, -1) # -> 2, 2560
        x = self.fc(x)
        return x
class RsnaLstm2(nn.Module):
    def __init__(
        self,
        num_classes=1,
        n_instance=5,
        ...
        self.lstm = nn.LSTM(nc*2, nc, num_layers=2, dropout=0, bidirectional=True, batch_first=True)
        self.head = nn.Sequential(
            nn.Linear(nc*2, nc//2),
            nn.BatchNorm1d(nc//2),
            nn.Dropout(0.1),
    ...
    def forward(self, x):
        ...
        x = self.pool(x) # -> 10, 1536, 1, 1
        x = x.view(bs, n, -1) # -> 2, 5, 1536
        x, _ = self.lstm(x) # -> 2, 5, 1536

        x = x.contiguous().view(bs * n, -1) # -> 10, 1536
        x = self.head(x) # -> 10, 3
        x = x.view(bs, -1).contiguous() # 2, 15
        return x
class RsnaGru2(nn.Module):
    def __init__(
        self,
        num_classes=1,
        n_instance=5,
        ...
        # self.pool = nn.AdaptiveMaxPool2d((1,1))
        # self.lstm = nn.LSTM(nc*2, nc, num_layers=2, dropout=0, bidirectional=True, batch_first=True)
        self.gru = nn.GRU(nc*2, nc, 2, batch_first=True, bidirectional=True)
        self.head = nn.Sequential(
            nn.Linear(nc*2, nc//2),
            nn.BatchNorm1d(nc//2),
            nn.Dropout(0.1),
...
class RSNA2ndModel2backbone(nn.Module):
    def __init__(
        self,
        num_classes=1,
        center_loss_feat_dim=512,
        ...
        self.head = nn.Sequential(
            Flatten(),
            nn.Linear(nc+nc2, self.center_loss_feat_dim),
            nn.ReLU(inplace=True),
        )
        self.fc = nn.Linear(self.center_loss_feat_dim, num_classes)
    ...
    def forward(self, x):
        ...
        bs, n, ch, w, h = x.shape # 8, 5, 3, 384, 384
        x = x.view(bs * n, ch, w, h) # 40, 3, 384, 384
        x2 = self.encoder2(x) # 40, 2048, 20, 20
        x = self.encoder(x) # 40, 2048, 20, 20
        bs2, ch2, w2, h2 = x.shape
...
class RsnaLstmXcit(nn.Module):
    ...
    def forward(self, x):
        ...
        x = self.fc(x)
        return x
class RsnaLstm4classes(nn.Module):
    def __init__(
        self,
        num_classes=1,
        n_instance=45,
    ...
    def forward(self, inputs):
        # inputs: liver, spleen, kidney, bowel
        xs = []
        for x_n, x in enumerate(inputs):
            bs, n, ch, w, h = x.shape
            x = x.view(bs * n, ch, w, h)
            x = self.encoder(x)
            x = self.pool(x)
            x = x.view(bs, n, -1)
            x, _ = self.lstm(x)
...
class MIL4classes(nn.Module):
    def __init__(
        ...
        self.fc = nn.Linear(self.center_loss_feat_dim*4, num_classes)
    ...
    def forward(self, inputs):
        ...
        for x_n, x in enumerate(inputs):
            ...
            # x.size() # 16, 384, 135, 27
            xs.append(x)
        liver = self.liver(xs[0])
        spleen = self.spleen(xs[1])
        kidney = self.kidney(xs[2])
        bowel = self.bowel(xs[3])
        x = torch.cat([liver, spleen, kidney, bowel], 1)
        x = self.fc(x)
        return x

class MIL4classes3(nn.Module):
    def __init__(
        self,
        num_classes=1,
        n_instance=45,
        ...
        self.fc = nn.Linear(nc*2*4, num_classes)
...
class MIL4classes2(nn.Module):
    def __init__(
        ...
        self.encoder_bowel = copy.deepcopy(self.encoder_liver)
        self.liver = nn.Sequential(
                AdaptiveConcatPool2d(),
                Flatten(),
                nn.Linear(2 * nc, self.center_loss_feat_dim),
    ...
    def forward(self, inputs):
        liver, spleen, kidney, bowel = inputs
        # print(liver.shape)
        bs, n, ch, w, h = liver.shape # 8, 5, 3, 384, 384
        liver = liver.view(bs * n, ch, w, h) # 40, 3, 384, 384
        liver = self.encoder_liver(liver) # 40, 2048, 20, 20
        bs2, ch2, w2, h2 = liver.shape
        liver = (
            liver.view(-1, n, ch2, w2, h2)
            .permute(0, 2, 1, 3, 4)
            .contiguous()
            .view(bs, ch2, n * w2, h2)
        )  # x: bs x C' x N W'' x W''
        liver = self.liver(liver)


        bs, n, ch, w, h = spleen.shape # 8, 5, 3, 384, 384
        spleen = spleen.view(bs * n, ch, w, h) # 40, 3, 384, 384
        spleen = self.encoder_liver(spleen) # 40, 2048, 20, 20
        bs2, ch2, w2, h2 = spleen.shape
        spleen = (
            spleen.view(-1, n, ch2, w2, h2)
            .permute(0, 2, 1, 3, 4)
            .contiguous()
            .view(bs, ch2, n * w2, h2)
        )  # x: bs x C' x N W'' x W''
        spleen = self.spleen(spleen)


        bs, n, ch, w, h = kidney.shape # 8, 5, 3, 384, 384
        kidney = kidney.view(bs * n, ch, w, h) # 40, 3, 384, 384
        kidney = self.encoder_liver(kidney) # 40, 2048, 20, 20
        bs2, ch2, w2, h2 = kidney.shape
        kidney = (
            kidney.view(-1, n, ch2, w2, h2)
            .permute(0, 2, 1, 3, 4)
            .contiguous()
            .view(bs, ch2, n * w2, h2)
        )  # x: bs x C' x N W'' x W''
        kidney = self.kidney(kidney)


        bs, n, ch, w, h = bowel.shape # 8, 5, 3, 384, 384
        bowel = bowel.view(bs * n, ch, w, h) # 40, 3, 384, 384
        bowel = self.encoder_liver(bowel) # 40, 2048, 20, 20
        bs2, ch2, w2, h2 = bowel.shape
        bowel = (
            bowel.view(-1, n, ch2, w2, h2)
            .permute(0, 2, 1, 3, 4)
            .contiguous()
            .view(bs, ch2, n * w2, h2)
        )  # x: bs x C' x N W'' x W''
        bowel = self.bowel(bowel)
...
class RsnaLstmWithMetaCols(nn.Module):
    def __init__(
        self,
        num_classes=1,
        n_instance=5,
        ...
        hidden_dim = 256
        self.lstm = nn.LSTM(nc*2, hidden_dim, num_layers=2, dropout=0, bidirectional=True, batch_first=True)
        self.meta_col_n = meta_col_n
        # self.mlp = nn.Sequential(
        #     nn.BatchNorm1d(meta_col_n),
        #     nn.Linear(meta_col_n, 128),
        #     nn.BatchNorm1d(128),
        #     nn.Dropout(p=dropout_ratio),
        #     nn.ReLU(),
        #     nn.Linear(128, 256),
        # )
        # self.fc = nn.Linear(hidden_dim*2*n_instance + 256*n_instance, num_classes)
        self.fc = nn.Linear(hidden_dim*2*n_instance + 2*n_instance, num_classes)
    ...
    def forward(self, x):
        ...
        x = x.view(bs * n, ch, w, h) # 40, 3, 384, 384
        x = self.encoder(x) # 40, 2048, 20, 20
        x = self.pool(x) # 40, 2048
        x = x.view(bs, n, -1) # 8, 5, 2048
        x, _ = self.lstm(x) # 8, 5, 2048
        x = x.reshape(bs, -1) # 8, 10240
        # meta = meta.view(bs * n, -1) # 40, 2
        # meta = self.mlp(meta) # 40, 256
        # meta = meta.reshape(bs, -1) # 8, 1280
        meta = meta.view(bs, -1) # 8, 2*5
        x = torch.cat([x, meta], 1) # 8, 10240 + 1280
        x = self.fc(x)
        return x

class Rsna1dcnn(nn.Module):
    def __init__(
        self,
        num_classes=1,
        n_instance=5,
        ...
        hidden_dim = 256
        self.cnn1d = nn.Conv1d(nc*2, hidden_dim*2, kernel_size=3, stride=1, padding=1)
        # self.cnn1d = nn.Conv1d(15, hidden_dim*2, kernel_size=3, stride=1, padding=1)
        self.fc = nn.Linear(hidden_dim*2*n_instance, num_classes)
    ...
    def forward(self, x):
        ...
        x = self.pool(x)
        x = x.view(bs, -1, n)
        x = self.cnn1d(x) # (B, seq_len, hidden_dim*2)
        x = x.reshape(bs, -1)
        x = self.fc(x)
        return x

class MultiInstanceCNNModelRetrainExptPool(nn.Module):
    def __init__(
        self,
        num_classes=1,
        n_instance=5,
    ...
    def forward(self, x):
        ...
        bs, n, ch, w, h = x.shape
        x = x.view(bs * n, ch, w, h)  # x: bs*n, C, W, W
        x = self.encoder(x)  # x: bs*n, C', W', W'
        # st()
        bs2, ch2, w2, h2 = x.shape
        x = (
            x.view(-1, n, ch2, w2, h2)
        ...
        mean_ = x.mean(2).mean(2)
        max_ = x.max(2)[0].max(2)[0]
...
class HMSModel2(nn.Module):
    def __init__(
        ...
        self.model2 = model2
        self.model3 = model3
        in_features = model0.head.in_features

        self.fc = nn.Linear(in_features*4, num_classes)

    def forward(self, inputs):
        x0, x1, x2, x3 = inputs[:, 0], inputs[:, 1], inputs[:, 2], inputs[:, 3]
        # print(f'{x0.shape=}, {x1.shape=}, {x2.shape=}, {x3.shape=}' )
        x0 = self.model0(x0)
        x1 = self.model1(x1)
        x2 = self.model2(x2)
        x3 = self.model3(x3)

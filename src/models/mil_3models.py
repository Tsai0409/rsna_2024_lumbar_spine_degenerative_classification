...
class ABMIL(nn.Module):
    def __init__(self, feature_extractor, num_classes, feat_dim=512, attn_dim=128):
        super().__init__()
        self.feature_extractor = feature_extractor
        self.feature_dim = self.feature_extractor.num_features

        self.downlinear = nn.Sequential(nn.Linear(self.feature_dim, feat_dim), nn.ReLU())
    ...
    def forward(self, x):
        ...
        x = self.downlinear(x) # (n_instance, in_features)
...
class ABMIL2(nn.Module):
    def __init__(self, feature_extractor, num_classes, num_images, feat_dim=512, attn_dim=128):
        super().__init__()
        self.feature_extractor = feature_extractor
        self.feature_dim = self.feature_extractor.num_features
    ...
    def forward(self, x):
        ...
        x = x.view(batch_size * num_images, c, h, w)
        x = self.feature_extractor(x)
        x = self.downlinear(x)  # (batch_size * num_images, feat_dim)
        
        A_V = self.attention_V(x)
        A_U = self.attention_U(x)
        A = self.attention_weights(A_V * A_U)
        A = A.view(batch_size, num_images, -1)
        A = torch.softmax(A, dim=1)
        
        x = x.view(batch_size, num_images, -1)
        x = torch.bmm(A.transpose(1, 2), x).squeeze(1)
...
class TransMIL(nn.Module):
    ...
    def forward(self, x):
        ...
        h = h.unsqueeze(0).float()  # [B, n, 1024]

...
@register_model
def _nextvit_small(pretrained=False, pretrained_cfg=None, **kwargs):
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 10, 3], path_dropout=0.1, **kwargs)
    return model


@register_model
def _nextvit_base(pretrained=False, pretrained_cfg=None, **kwargs):
    model = NextViT(stem_chs=[64, 32, 64], depths=[3, 4, 20, 3], path_dropout=0.2, **kwargs)
    return model


@register_model
def _nextvit_large(pretrained=False, pretrained_cfg=None, **kwargs):
...
class NextVitNet(nn.Module):
    def __init__(self, model_name, pretrained=True, num_classes=1):
        ...
        if pretrained:
            path = f'/groups/gca50041/ariyasu/nextvit_weights/{model_name}_in1k_384.pth'

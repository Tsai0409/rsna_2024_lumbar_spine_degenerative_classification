...
def weighted_cross_entropy_loss(labels,
    ...
    negative_weights = negative_weights.cuda()
    positive_weights = positive_weights.cuda()
    weight_dependent_factor = (negative_weights.cuda() + (positive_weights - negative_weights)*labels)

    return (negative_weights * (logits - labels * logits) + weight_dependent_factor * softerm_plus)

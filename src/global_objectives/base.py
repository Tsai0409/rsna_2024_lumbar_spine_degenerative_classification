...
class BaseLoss(nn.Module):
    ...
    def __init__(self, at_target, at_target_type, num_labels,
        ...
        if loss_type == 'cross_entropy':
            self.loss_func = weighted_cross_entropy_loss
        else:
            self.loss_func = weighted_hinge_loss
        ...
        if self.is_auc:
            ...
            _at_target_values, self.delta = build_anchors(
    ...
    def forward(self, logits, labels,
        ...
        logits, labels, weights = \
            validate_prepare_logits_labels_weights(
        ...
        positive_weights, negative_weights = \
            prepare_positive_negative_weights(
        ...
        weighted_loss = self.loss_func(
            _labels,
            _logits - self.biases.cuda(),
            positive_weights=positive_weights,
            negative_weights=negative_weights,
        )
        # import pdb;pdb.set_trace()

        loss = _weights * weighted_loss - lambda_term.cuda()

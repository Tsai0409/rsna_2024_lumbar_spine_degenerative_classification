...
def rsna_qishen(size):
    return {
        'train': A.Compose([
            ...
            A.CoarseDropout(max_height=int(size[0]*0.5), max_width=int(size[1]*0.5), max_holes=1, p=0.5),
            A.Normalize(),
            ToTensorV2(),
        ]),
        'val': A.Compose([
...
def rsna_2024_aug_v1(size):
    return {
        ...
        "val": A.Compose([
            # A.SmallestMaxSize(max_size=size[0]),
            # A.CenterCrop(size[0], size[1], always_apply=True, p=1),
            A.Resize(size[0], size[1]),
            A.Normalize(),
            ToTensorV2(),
        ], additional_targets={'image1': 'image', 'image2': 'image', 'image3': 'image', 'image4': 'image'}),
...
def nodoca_aug_with_norm_with_crop(size):
    return {
            'train': A.Compose([
                A.RandomResizedCrop(
                    size[0],
        ...
        'val': A.Compose([
            A.CenterCrop(1080, 1920 - 400, always_apply=True, p=1),
            A.Resize(size[0], size[1]),
            A.Normalize(),
            ToTensorV2(),
        ])
...
def inf_isp_aug_v3(size):
    return {
        'val': A.Compose([
            A.Resize(size[0], size[1]),
            A.RandomBrightnessContrast(brightness_limit=0.1, contrast_limit=0.1, p=1),
            # 色彩の変化を模倣
            A.HueSaturationValue(hue_shift_limit=5, sat_shift_limit=5, val_shift_limit=20, p=1),
            # ノイズの追加
...
def hms_aug_v7(size):
    return {
            'train': A.Compose([
                ...
                A.HorizontalFlip(p=0.5),
                # A.VerticalFlip(p=0.5),
                # A.Cutout(max_h_size=int(size[0] * 0.35), max_w_size=int(size[1] * 0.35), num_holes=3, p=0.5),
                # A.CoarseDropout(max_height=int(size[0]*0.35), max_width=int(size[1]*0.35), max_holes=3, p=0.5),
                # A.Normalize(),
                # ToTensorV2(),
            ], p=1),
        'val': A.Compose([
...
def hms_aug_v12_transpose(size):
    params = {
        "num_masks_x": (4, 20),
        "num_masks_y": (4, 20),
        "mask_y_length": (5, 20),
...
def rsna_v11_aug_str(size):
    return {
        'train': A.Compose([
             ...
             A.Sharpen(p=0.5),
             A.CoarseDropout(max_height=int(size[0]*0.1), max_width=int(size[1]*0.1), max_holes=10, p=0.5),
             A.Normalize(),
             ToTensorV2(),
        ]),
        'val': A.Compose([
